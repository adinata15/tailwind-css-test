...
export default {
  ...
  created() {
    ...
    this.$on(
      'cv:panel-control-beforeDestroy',
      this.onCvPanelControlBeforeDestroy
    );
  ...
  methods: {
    hasRail(_ctrl, _panel) {
      ...
      if (ctrl) {
        ...
        if (panel) {
          ctrl = this.panelControllers.find(
    ...
    onCvPanelControlBeforeDestroy(srcComponent) {
      const index = this.panelControllers.findIndex(
    ...
    onCvPanelControlToggle(srcComponent, force) {
      const foundIndex = this.panels.findIndex(
        (item) => item.id === srcComponent.ariaControls
      );
      if (foundIndex > -1) {
        const newValue =
          force !== undefined ? force : !srcComponent.panelExpanded;

        for (const index in this.panels) {
          this.panels[index].panelExpanded = false;
        }
        for (const index in this.panelControllers) {
    ...
    onCvPanelFocusout(srcComponent, srcEvent) {
      const found = this.panelControllers.find(
